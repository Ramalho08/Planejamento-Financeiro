...
const KEY='rf_v21_5_2_rollback_stable';
...
on('backupBtn','click',()=>download('ramalho-finance-v21-5-2-rollback-backup.json',JSON.stringify(state,null,2),'application/json'));
...
function exportCsv(){const rows=['Mes,Data,Tipo,Categoria,Descricao,Valor'];state.tx.forEach(t=>rows.push([t.month,t.date,t.type,t.category,'"'+t.description+'"',t.amount].join(',')));download('ramalho-finance-v21-5-2-rollback.csv',rows.join('\n'),'text/csv');}
...
function initStablePlusButtons(){
  ...
  if(clean&&!clean.dataset.ready){
    ...
    clean.addEventListener('click',function(){
      ...
      location.replace(location.href.split('?')[0]+'?v=2152rollback&t='+Date.now());
    });
  }
  ...
  if(backup&&!backup.dataset.ready){
    ...
    backup.addEventListener('click',function(){
      download('ramalho-finance-v21-5-2-rollback-backup.json',JSON.stringify(state,null,2),'application/json');
    });
  }
}
