...
var KEY='rf_v16_5_ultimate_stable';
...
on('backupBtn','click',function(){download('ramalho-finance-v16-5-backup.json',JSON.stringify(state,null,2),'application/json');});
...
function render(){
  ...
  renderBars(t);renderCategories();renderTopExpenses();renderMonthlyCompare();renderTransactions();renderWallets();renderCards();renderBudgets();renderSubs();renderPlan();renderCustomCategories();renderCalendar();renderGoals();renderInvests();renderChecklist(t);renderCloud();renderSummary();
}
...
function exportCsv(){var rows=['Mes,Data,Tipo,Categoria,Descricao,Valor,Recorrente'];state.tx.forEach(function(t){rows.push([t.month,t.date,t.type,t.category,'"'+t.description+'"',t.amount,!!t.recurring].join(','));});download('ramalho-finance-v16-5.csv',rows.join('\n'),'text/csv');}
