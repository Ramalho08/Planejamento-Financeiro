...
on('backupBtn','click',function(){download('ramalho-finance-v17-4-1-fix-backup.json',JSON.stringify(state,null,2),'application/json');});
...
function exportCsv(){var rows=['Mes,Data,Tipo,Categoria,Descricao,Valor,Recorrente'];state.tx.forEach(function(t){rows.push([t.month,t.date,t.type,t.category,'"'+t.description+'"',t.amount,!!t.recurring].join(','));});download('ramalho-finance-v17-4-1-fix.csv',rows.join('\n'),'text/csv');}
