...
function renderProfessionalCoreEngine(){
  ...
  <div class="alert core-good"><b>Finance Engine:</b> cálculos, índices, filtros e verificações centralizados no Supabase Cloud READY V2.</div>`;
}
...
function renderProfessionalCoreAudit(){
  ...
  const d = coreEngineData();
  const audit = [];
  audit.push(["Sistema","Supabase Cloud READY V2 ativo","core-good"]);
...
function renderSettings(){
  RF.$("systemInfo").innerHTML = `<div class="alert"><b>Versão:</b> Supabase Cloud READY V2</div><div class="alert"><b>Armazenamento:</b> localStorage</div><div class="alert"><b>Pronto para:</b> Firebase, IA Cloud e PDF/OCR</div>`;
}
...
function init(){
  try{
    ...
    renderAll();
    window.RF = RF;
    console.log("Ramalho Finance Supabase Cloud READY V2 iniciado.");
  }catch(err){
    console.error(err);
