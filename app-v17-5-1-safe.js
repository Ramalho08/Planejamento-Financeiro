...
on('backupBtn','click',function(){download('ramalho-finance-v17-5-1-safe-backup.json',JSON.stringify(state,null,2),'application/json');});
...
on('safeBackupBtn','click',function(){download('ramalho-finance-v17-5-1-safe-backup.json',JSON.stringify(state,null,2),'application/json');});
...
function render(){
  ...
  renderBars(t);renderCategories();renderTopExpenses();renderMonthlyCompare();renderPremiumCenter(t,score,saving,cardPct);renderPremiumPlus(t,score);renderUltraHealth(t,score,saving,cardPct);renderInfinityStats(t,score,saving,cardPct);renderEliteForecast(t,score,saving,cardPct);renderWealthCenter(t);renderFutureBills();renderCardElite();renderCoachPlan(t,score);renderGlobalSearch();renderStatistics(t,score);renderInvestPro();renderFavorites();renderRecentActions();renderNotifications(t);renderTimeline();renderGoalPro();renderDailyFlow();renderGoalInsights();renderAnalytics(t,score);renderCategoryRanking();renderDebts();renderAssets();renderAnnual(t);renderTransactions();renderWallets();renderCards();renderBudgets();renderSubs();renderPlan();renderCustomCategories();renderCalendar();renderGoals();renderInvests();renderChecklist(t);renderCloud();renderSummary();safeRender();
}
...
function exportCsv(){var rows=['Mes,Data,Tipo,Categoria,Descricao,Valor,Recorrente'];state.tx.forEach(function(t){rows.push([t.month,t.date,t.type,t.category,'"'+t.description+'"',t.amount,!!t.recurring].join(','));});download('ramalho-finance-v17-5-1-safe.csv',rows.join('\n'),'text/csv');}
