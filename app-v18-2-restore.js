...
on('backupBtn','click',()=>download('ramalho-finance-v18-2-restore-backup.json',JSON.stringify(state,null,2),'application/json'));
...
function exportCsv(){const rows=['Mes,Data,Tipo,Categoria,Descricao,Valor'];state.tx.forEach(t=>rows.push([t.month,t.date,t.type,t.category,'"'+t.description+'"',t.amount].join(',')));download('ramalho-finance-v18-2-restore.csv',rows.join('\n'),'text/csv');}
